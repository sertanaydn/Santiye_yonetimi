...
export default function WaybillListPage() {
    ...
    const handleResetStatus = async (waybill: any) => {
        if (!confirm(`${waybill.waybill_no || 'Bu'} irsaliyenin cari bağlantısını SIFIRLAMAK istiyor musunuz?\n\n(Bunu yaparsanız tekrar 'Cariye Aktar' butonu görünür olacaktır).`)) return;
        ...
        try {
            ...
            toast.success("İrsaliye durumu sıfırlandı. Tekrar aktarabilirsiniz.");
            fetchWaybills();
                                            ...
                                            <TableCell className="py-3 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {w.sync_status ? (
                                                        <Badge
                                                            variant="outline"
                                                            className="bg-green-50 text-green-600 border-green-200 text-[10px] cursor-pointer hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
                                                            onClick={() => handleResetStatus(w)}
                                                            title="Sıfırlamak için tıklayın"
                                                        >
                                                            Aktarıldı
                                                        </Badge>
                                                    ) : (
